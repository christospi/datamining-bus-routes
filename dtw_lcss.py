...
# Iterate over test_a1 rows
    for i, t_row in df_test.iterrows():
        t = threading.Thread(target=worker, args=(i, t_row,df,df_test))
        threads.append(t)
        t.start()

def worker( i, t_row,df,df_test):
    print 'Worker:',i
    test_list = []
    neighbours = []
    tlat= []
    ...
    gmap.draw('dtwMaps/test-' + str(i) + '.html')
    print "Test Trip ", i,"\n"
    for n in range(0,5):
        for g, grow in df[df['tripId'] == neighbours[n][0]].iterrows():
            ...
            gmap.draw('dtwMaps/dtwmap-tripID' + grow[1] +"test "+ str(i) +"n"+str(n)+ '.html')
            print "Neighbor ",n," \nJP_ID: ",grow[1]," \nDTW:",neighbours[n][1],"\n"
